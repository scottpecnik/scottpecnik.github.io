...
It's common practice in Elasticsearch to create [index templates](https://www.elastic.co/guide/en/elasticsearch/reference/current/indices-templates.html) in order to provide some type of organization for your data. The *puppet problem* arises with creating said index...they can only be created via REST call to a running instance of Elasticsearch.

Since we have the luxury of running Elasticsearch within a container, it would make logical sense to bake the index into an image. Except for that [doesn't work](https://stackoverflow.com/questions/35526532/how-to-add-an-elasticsearch-index-during-docker-build), and for good reason. Elasticsearch indices dictate the way data is stored, which in our case lives safely inside docker volumes. A volume would need to exist and be mounted if the index is to persist.
...
Once you've got the puppet agent installed, we'll generate a boilerplate project using the puppet module generator. This is as simple as typing the following into a terminal
...
By default a project contains the following folders with their respective purpose:
- examples - For demonstrating how to use your module, as well as providing the ability to test locally.
- manifests - For puppet manifests, typically a module written in the puppet declarative language will **define** types that can be declared by the consuming puppet project.
- spec -

I have added a few folders to my project:
- files - As part of my test code in the examples folder, I ship a sample Elasticsearch index template definition in JSON.
- lib - When writing ruby in a puppet module, it must be inside of a folder called lib.
...
Before delving into the code, it's necessary to first discuss **types** and **providers**. In the puppet language there are about 50 out of the box defined types.  I'm talking about **file**, **package**, **service**, etc. When creating a puppet module, we'll be defining our own type that can later be declared in the puppet language. For a type to actually do something, we must create a provider with the logic to manipulate said resource.

I like to think that types and providers are akin to models and controllers, respectively, in the Model View Controller (MVC) programming paradigm.

Let's start by building a folder structure under the lib folder that will support our new module code.  First, create a top level folder called **puppet**, with child folders **type** and **provider**.

![lib folder]({{ site.name }}/assets/images/creatingrubymod/lib_folder.png)

##### Type

Next, create a ruby file under the type folder. The name of the file is arbitrary, but for those that will come after us it's probably best to use the name of our type as the name of the file.
...
Finally, we round out the type with a list of parameters and properties.  The parameters will be passed into a resource via puppet code to help describe the resource type.  Note the existance of :namevar => true on the name parameter.  This tells puppet that the name of the resource when it is defined in puppet language is to be interpreted as the name parameter.  Alternatively, you could use an arbitrary name and explicitly provide a name parameter.  Below is what a declaration of our new type in puppet language would look like.
...
Now that we've defined what our type will look like, we need to write the logic that will control how the resource gets created.  Like before, below is the finished product, take a look and then scroll down for an explanation.
...
The code starts very similar to the type, declaring that we are a provider for the type **elasticsearch_template**.  Note that there are two class parameters. The first is the name of our ruby file that is the provider.  I know, I picked a very intuitive file name in **ruby**.  See the below screenshot for our final lib folder structure in order to get a better understanding.  The second param is that of a parent class.  This is an unnecessary step, but something I decided to do in order to keep the implementation simple from the perspective of the provider.  I'll explain better later.

![lib folder]({{ site.name }}/assets/images/creatingrubymod/expanded_lib_folder.png)

Next up are three standard methods:
- exists
- create
- destroy

Remember earlier when I mentioned ensureable and magic in the same sentence?  Since our type is ensureable, the puppet framework will automatically call the **exists** method during a puppet run in order to get the state of the resource. Puppet will then call either **create** or **destroy** based on what the implementer provides as the ensure in their resource declaration - hint: either **present** or **absent**.

Note the helper methods (doesTemplateExist, createTemplate, and deleteTemplate) that are called from within the **exists**, **create**, and **destroy**.  Having these is a matter of personal preference and not required. Also note how we reference parameters from the type using the following syntax:
...
And finally, you'll notice that within the helper methods we call another method called **rest**. This method exists in our parent class called **rest.rb**.  I'll skip the explanation of that implementation as it's more a function of ruby coding and not puppet module framework stuff. Below is a link to the final project in github where you can find the implementation of the **rest** class.

#### Testing your Module

Writing a module in ruby can be a little tedious at times. It's best to setup a local test bed within your module to easily measure quality. To do this, create a manifest (using any name you want) inside of the examples folder.  Below you can see that I've decided to continue on with my poor naming standards and use init.pp.  I've also provided a boilerplate index template in the files directory.

![test manifest]({{ site.name }}/assets/images/creatingrubymod/test_manifest.png)

My test code is pretty straightforward. First I write the boilerplate index template to my local filesystem so that it can be read by the REST call, then I define my new type with an ensure **present**. Note that the **servername** is set to localhost, which means that elasticsearch will have to be running on my local system for a proper test. That code is not shown here...
...
Finally, to run our test we simply apply the example manifest using the below command. Note that we need to tell puppet where to find our module by setting the modulepath parameter.  On my system the module's parent folder is **~/repositories**.
